...
def main(args):
    ...
    encoder_optimizer = torch.optim.Adam(model.parameters(), lr=args.learning_rate)
    decoder_optimizer = torch.optim.Adam(model.parameters(), lr=args.learning_rate)
    ...
    for epoch in range(args.epochs):
        ...
        for split in splits:
            ...
            for iteration, batch in enumerate(data_loader):
                ...
                #NLL_loss, KL_loss, KL_weight = loss_fn(logp, batch['target'],
                #                                       batch['length'], mean, logv, args.anneal_function, iteration, args.k,
                #                                       args.x0)
                #print("at iteration %i KL-weight is %0.3f" %(step, KL_weight))
                # try autoencoder by making KL_weight to zero
                if split == 'valid' or split == 'test':
                    KL_weight = 1
                    if epoch % 10 == 9:
                        tsv_file = open("mean_of_latent_space"+str(split)+str(epoch)+".tsv", "w")
                        tsv_writer = csv.writer(tsv_file, delimiter="\t")
                        meanVar = mean
                        for i in range(len(meanVar)):
                            for dim in meanVar[i]:
                                tsv_writer.writerow(dim)
                        tsv_file.close()
                    #print("KL_weight not at train: ", KL_weight)
                loss = (NLL_loss + KL_weight * KL_loss)/batch_size

                # backward + optimization
                if split == 'train':
                    # fix the decoder for the first 1 epoch
                    if epoch <= 0:
